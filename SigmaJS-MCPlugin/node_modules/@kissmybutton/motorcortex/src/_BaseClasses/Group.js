const TimedIncident = require('./TimedIncident');
...
const _CONST = require('../configuration/_CONST');
 ...
 */
class Group extends TimedIncident {
    constructor(props={}, oldProps = null) {
        let attrsToPass, propsToPass;
    ...
    */
    _getChannel(mc_plugin_npm_name){
    ...
    */
    removeEmtpyGroups(){
        ...
        for(let i=0; i<incidentsToRemove.length; i++){
            this.removeIncident(incidentsToRemove[i]);
        }
    }
    ...
    */
    _getDirectParent(incidentId){
        if(this.incidentsById.hasOwnProperty(incidentId)){
            return this;
        } else {
            for(let i=0; i<this.incidents.length; i++){
                if(this.incidents[i].incident.hasIncidents){
                    let directParent = this.incidents[i].incident._getDirectParent(incidentId);
    ...
    */
    getIncidentsByChannel(adjustMillisecond=0){
        ...
        for(let i=0; i<this.incidents.length; i++){
            const incidentsToAdd = this.incidents[i].incident.getIncidentsByChannel(adjustMillisecond + this.incidents[i].millisecond);
    ...
    */
    getProjectedIncidents(adjustMillisecond=0){
        let projectedIncidents = [];
        for(let i=0; i<this.incidents.length; i++){
            const incidentsToAdd = this.incidents[i].incident.getProjectedIncidents(adjustMillisecond + this.incidents[i].millisecond);
     ...
     */
    getDurationAfterIncidentAffected() {
        const durationBeforeChange = this.duration*1;
        const currentMillisecondBeforeChange = this.runTimeInfo.currentMillisecond*1;
        const stateBeforeChage = this.state + '';
     ...
     */
    getDurationAfterIncidentAdded(startMillisecond, incidentDuration) {
        const durationBeforeChange = this.duration*1;
        const currentMillisecondBeforeChange = this.runTimeInfo.currentMillisecond*1;
    ...
    */
    recalcState(targetMillisecondDeltaFromStart) {
        super.recalcState(targetMillisecondDeltaFromStart);
        for (let i = 0; i < this.incidents.length; i++) {
            const incident = this.incidents[i].incident;
            const startMillisecond = this.incidents[i].millisecond;
    ...
    getIncidentMillisecond(incidentId) {
    ...
    exportState(props={}){
        ...
        let state = super.exportState(props);
    ...
    setCurrentMillisecond(millisecond){
        ...
        for(let i=0; i<this.incidents.length; i++){
            const childIncident = this.incidents[i];
    ...
    */
    getIncidentById(id) {
        if(this.incidentsById.hasOwnProperty(id)){
            ...
            for(let i=0; i<this.incidents.length; i++){
                if(this.incidents[i].incident.hasIncidents){
                    let incident = this.incidents[i].incident.getIncidentById(id);
                    if(incident != null){
                        return incident;
                    }
                }
    ...
    */
    expandSceneToTheLeft(howManyMilliseconds, sceneId){
        // First we identify the expansion within the context of the parent (this) Scene
        const currentStartMillisecondOfTheChildScene = this.getIncidentMillisecond(sceneId);
        const newStartMillisecondOfTheChildScene = howManyMilliseconds - currentStartMillisecondOfTheChildScene;
        ...
        if(newStartMillisecondOfTheChildScene <= 0){
            const rootScene = this;
            const changeResult = this.changeIncidentsStartMillisecond(-newStartMillisecondOfTheChildScene, sceneId);
            const undo = () => {
                this.changeIncidentsStartMillisecond(changeResult.oldMillisecond, sceneId);
            }
            return {
                result: true,
                undo: undo,
        ...
        const parentDecision = this.parentSceneObject.expandSceneToTheLeft(newStartMillisecondOfTheChildScene, this.id);
    ...
    */
    expandStartPoint(byMilliseconds){
        for(let i=0; i<this.incidents.length; i++){
            const incident = this.incidents[i];
    ...
    */
    changeIncidentsStartMillisecond(newmillisecond, id){
        const keyframe_sec_key = this.incidentsKeyframeKeys[id];
        ...
        let oldMillisecond; // the previous millisecond of the edited incident before the edit
        for (let i = 0; i < this.incidents.length; i++) {
        ...
        const incident = this.incidentsById[id];
        const new_keyframe_sec_key = conf.keyframe_sec_key + newmillisecond;
        ...
        const funct = (sceneMillisecond, previousTimeStamp, sceneState) => {
    ...
    _prepareInicdentForAddition(incident){
        if(!incident.props.hasOwnProperty('selector') && !incident.hasIncidents){ // if the incident is not a group and it has no selector proceed normally
            return incident;
        ...
        const theDOMAwareIncident = new DOMAwareIncident(incident);
    ...
    flashDOM(){
        ...
        for(let i=0; i<this.incidents.length; i++){
            const flashResult = this.incidents[i].incident.flashDOM();
    ...
    addPassiveIncident(incident, millisecond, sceneId, type="all-or-nothing"){
    ...
    addIncident(incident, millisecond, sceneId, type="all-or-nothing", autoGenerated=false) {
        ...
        let originalMillisecond = millisecond*1; // a backup variable holding the original millisecond passed as
        ...
        if (!sceneId || sceneId === this.id) {
            ...
            let flashIncidentsDOMAfterEnter = false;
            ...
            const additionCheck = this.checkAddition(incident, millisecond, type);
            ...
            const keyframe_sec_key = conf.keyframe_sec_key + millisecond;
            ...
            const funct = (currentMillisecond, previousTimeStamp, sceneState) => {
                if(incident.speed > 0){
                    if (currentMillisecond >= millisecond && currentMillisecond <= millisecond + incident.duration
                        || incident.state === 'idle' && currentMillisecond > millisecond) {
                        // helper.log('greater');
                        incident.play({ startFrom: currentMillisecond - millisecond, previousTimeStamp: previousTimeStamp });
                    }
                } else {
                    if (currentMillisecond >= millisecond && currentMillisecond <= millisecond + incident.duration
                        || incident.state === 'completed' && currentMillisecond < millisecond) {
                        // helper.log('greater');
                        incident.play({ startFrom: currentMillisecond - millisecond, previousTimeStamp: previousTimeStamp });
                    }
                }
            ...
            if(flashIncidentsDOMAfterEnter){
                const result = incident.flashDOM();
            ...
            for (let i = 0; i < this.incidents.length; i++) {
                if(this.incidents[i].incident.hasIncidents) {
                    if (
                        this.incidents[i].incident.addIncident(
                            incident,
                            millisecond - this.incidents[i].millisecond,
                            sceneId
                        ).result === true
                    ) {
                        return {
    ...
    editIncident(id, newmillisecond) {
        let expansionValidation = null;
        let originalMillisecond = newmillisecond*1;

        if(!helper.isString(id)){
            helper.error(`editIncident method of Group expects as its first argument the id of the Incident to edit in string format. ${typeof id} passed`);
            return {
                result: false,
                conflicts: [
                    {
        ...
        if (this.incidentsById[id]) {
            const existingMillisecond = this.getIncidentMillisecond(id);
            const millisecondDelta = newmillisecond - existingMillisecond;
            ...
            const editCheck = this.checkEdit(this.incidentsById[id], millisecondDelta);
            ...
            const editIncidentStartResult = this.changeIncidentsStartMillisecond(newmillisecond, id);
            ...
            for (let i = 0; i < this.incidents.length; i++) {
                if(this.incidents[i].incident.hasIncidents) {
                    const result = this.incidents[i].incident.editIncident(id, newmillisecond - this.incidents[i].millisecond);
                    if (result.result === true) {
                        return {result:true};
                    } else if(result.hasOwnProperty('errors')){ // if the result.result is false but it contains no errors that means the Incident we're looking for just not belong to 
    ...
    removeIncident(id, props={}) {
        if (this.incidentsById[id]) {
            const deleteCheck = this.checkDelete(id, props);
            ...
            for (let i = 0; i < this.incidents.length; i++) {
                if(this.incidents[i].incident.hasIncidents) {
                    if ((this.incidents[i].incident.removeIncident(id)).result === true) {
                        return {result:true};
                    }
                }
            } // if it reaches the end of the loop without having returned true
    ...
    */
    resizeIncident(incidentId, newDuration){
        const existingIncident = this.getIncidentById(incidentId);
    ...
    */
    editIncidentAttributes(incidentId, attrs){
        const directParent = this._getDirectParent(incidentId);
        if(directParent === null){
            return {
                result: false,
    ...
    */
    _editChildIncidentAttributes(incidentId, attrs){
        const incidentToEdit = this.getIncidentById(incidentId);
        const existingIncidentDefinition = incidentToEdit.exportState();
        const millisecond = this.getIncidentMillisecond(incidentId);
        this.removeIncident(incidentId);
        let newIncidentProps = Object.assign({}, existingIncidentDefinition.props);
        const newIncident = new existingIncidentDefinition.Incident(attrs, newIncidentProps);
        const result = this.addIncident(newIncident, millisecond);
        if(result.result === false){
            const myOldIncident = new existingIncidentDefinition.Incident(existingIncidentDefinition.attrs, newIncidentProps);
    ...
    **/
    ...
    /**
           ...
           */
    incidentStateChange(id, oldState, newState, props) {
        if (newState === 'clipBlockingWaiting' || newState === 'sceneBlockingWaiting') {
            ...
            for (let i = 0; i < this.incidents.length; i++) {
                ...
                //     this.incidents[i].incident.id + '.state = ' + this.incidents[i].incident.state
                // );
                if (this.incidents[i].incident.state != 'idle') {
                    allCompleted = false;
                    break;
                }
            }
    ...
    */
    removeBlock(props){
        let newBlockingWaitings = [];
        let found = false;
        for(let i=0; i<this.blockingWaitings.length; i++){
            const bWaiting = this.blockingWaitings[i];
    ...
    **/
    getRootCurrentMillisecondDeltaFromIncidentsStart(incidentId, extraDelta=0){
        // first step find the start millisecond of the incident
        const incidentMillisecond = this.getIncidentMillisecond(incidentId);
        if(!this.checkParentScene()){
     ...
     */
    onAddIncident(incident, millisecond) {}

    onEditIncident(incident, oldMillisecond, newMillisecond) {}
